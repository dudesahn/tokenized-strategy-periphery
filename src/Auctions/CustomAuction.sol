...
// ideas
// allow operator to change a flag for per-auction startingPrice?
// have flag to only allow cowswap to take auctions?
...
contract Auction is Governance2Step, ReentrancyGuard {
    ...
    uint256 public customHalfLife; // aim for ~0.3% per minute
    // 0.3% per minute: 0.997 * 1e27
    // 0.15% per minute: 0.9985 * 1e27
    
    /// @notice Whether to use our adjustable minute half life or the constant legacy version (~1.1%).
    bool public useCustomHalfLife;
    
    /// @notice Flag to require that CoWSwap's settlement contract takes 
    bool public requireCowswap;

    /// @notice Used for the price decay.
    uint256 internal constant LEGACY_MINUTE_HALF_LIFE =
    ...
    function initialize(
        ...
        // set our custom half-life, off by default
        customHalfLife = 0.997 * 1e27
     ...
     * @param _timestamp The specific timestamp for calculating the price.
     * @return price The calculated price scaled to 1e18.
     */
    function _price(
        uint256 _kicked,
        uint256 _available,
        ...
        uint256 minutesComponent;
        uint256 initialPrice;

        if (useCustomHalfLife) {
            minutesComponent = Maths.rpow(
                customHalfLife,
                (secondsElapsed) / 30 // adjust price every 30 seconds
            );
            initialPrice = Maths.wdiv(startingPrice * 1e18, _available);

            price = (initialPrice * minutesComponent) / 1e27;
        } else {
    ...
    /**
     * @notice Sets the custom minute half life for the auction
     * @dev This is how much the price decays each minute. For example, 0.97e27 will decay 3% per minute.
     * @param _customHalfLife The new minute half life.
     */
    function setCustomHalfLife(
        uint256 _customHalfLife
    ) external virtual onlyGovernance {
        require(_customHalfLife != 0, "half life");

        customHalfLife = _customHalfLife;

        emit UpdatedMinuteHalfLife(_customHalfLife);
    }

    /**
     * @notice Set whether auctions are open or cowswap-only.
     * @dev Only do this if you know cowswap can properly solve your tokens.
     * @param _requireCowswap Whether to only allow cowswap to settle auctions.
     */
    function setRequireCowswap(
        uint256 _requireCowswap
    ) external virtual onlyGovernance {
        requireCowswap = _requireCowswap;

        emit UpdatedRequireCowswap(_requireCowswap);
    }
    ...
    function _take(
        ...
        // check if we've set to only allow cowswap to take auctions
        require(!requireCowswap, "!cowswap");
